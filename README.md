...
這是一個整合台灣各大常見求職平台的爬蟲專案，我們利用業界常見的架構定期抓取職缺資料並將之寫入資料庫。
...
初學者可能會想：「直接寫一個 Python script 一次把所有職缺資訊抓下來不就好了嗎？」

是可以，但當你面對以下情境時就會卡住：
- **資料量大**：對不同資料領域相關職缺進行爬蟲，一台電腦跑一整天還沒跑完
- **需要容錯**：抓到一半部分程式出錯，整支程式也崩潰，使前面抓取到的資料也跟著消失
...
專案根目錄有很多 `.yml`，這些是 Docker Compose 設定檔，用來一鍵啟動各種服務。根據其功能幫使用者分類如下：
